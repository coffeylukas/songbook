...
### M8 — Add secrets everywhere they're needed
- **Status:** todo
- **Description:** Once M3/M4 credentials exist, add them to: (a) a local `.env.local` (never
  ...
  Secrets and variables → Actions (for CI to run integration tests / builds).
- **Definition of done:** All three locations have matching, current values.
- **Dependencies:** M2, M3, M4.

### M9 — Confirm CCLI (or equivalent) license status
- **Status:** todo
...
### M10 — (Optional, later) Point a custom domain at Vercel
- **Status:** todo
